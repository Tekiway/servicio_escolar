 ...
 * Elimina una materia por ID del archivo materias.json
 */
...
if (!file_exists($jsonFile)) {
    echo json_encode(['success' => false, 'message' => 'No hay materias registradas.']);
    exit;
}
...
$nuevas   = array_filter($materias, fn($m) => $m['id'] !== $idEliminar);

if (count($nuevas) === count($materias)) {
